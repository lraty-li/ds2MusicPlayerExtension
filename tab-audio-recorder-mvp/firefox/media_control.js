...
async function handleBrowserControl(message, fallbackTabId, preferredFrameId) {
  ...
  try {
    if (typeof preferredFrameId === "number") {
      await injectPageControl(tabId, { frameIds: [preferredFrameId] });
      const direct = summarizeControlResults(command,
        await callPageControl(tabId, { frameIds: [preferredFrameId] }, "control", command));
