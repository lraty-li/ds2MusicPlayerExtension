...
async function fetchJacket(url) {
  ...
  try {
    const backgroundFirst = isFirefoxCaptureSender();
    if (backgroundFirst) {
      const backgroundResponse = await fetchJacketViaBackground(url);
    ...
    if (!response.ok && !backgroundFirst) {
      const backgroundResponse = await fetchJacketViaBackground(url);
      if (backgroundResponse) return backgroundResponse;
    }
    return response;
...
function isFirefoxCaptureSender() {
  return typeof window !== "undefined" && !!window.__ds2FirefoxCapture;
}

async function fetchJacketViaBackground(url) {
  if (typeof chrome === "undefined" || !chrome.runtime ||
    typeof chrome.runtime.sendMessage !== "function") {
    return null;
  }
  try {
    const payload = await chrome.runtime.sendMessage({ type: "fetch-jacket", url });
    if (!payload || !payload.ok || !payload.buffer) return null;
    return {
      ok: true,
      ...
      async blob() {
        return new Blob([payload.buffer], { type: payload.mime || "" });
      }
    };
  } catch (_) {
    return null;
