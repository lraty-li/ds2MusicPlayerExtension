...
Firefox adapter capture
  -> site adapter captureSource()
  -> page AudioContext PCM chunks
  -> content WebSocket sender
  -> ws://127.0.0.1:47832
```
...
2. 在 Chrome/Edge 扩展页加载本目录；Firefox 使用发布包内
   `browser-extension-firefox\manifest.json` 临时加载。
3. 打开一个播放音乐的标签页。
4. 点击扩展图标。
...
扩展收到后会按当前捕获 tab 执行控制，并以游戏内播放器状态为准设置浏览器播放状态。游戏说暂停时只暂停仍在播放的媒体；游戏说恢复时只恢复仍处于暂停状态的媒体，避免把控制做成反复 toggle。
...
adapters/spotify.js: Spotify Web 播放栏和媒体元素
adapters/bilibili.js: Bilibili 主播放器 video
...
扩展只为 YouTube、网易云、Spotify 和 Bilibili 请求站点权限；Spotify 需要早期 hook 来兼容 Media Session 控制。更新扩展后浏览器可能要求重新确认权限。
