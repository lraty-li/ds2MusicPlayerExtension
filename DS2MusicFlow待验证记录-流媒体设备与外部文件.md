...
- 同时，新的最小验证还补上了另一条运行时限制：
  - 这轮直接选中的 `sub_14206A490 / sub_14206FEF0` 虽然安装成功
  - 但在真实试听、正式播放、下一首流程里都没有出现命中日志
  - 因而当前还不能把这轮选中的两个函数落点，直接当成真实音乐链已经过的运行时节点
- 但目前还没有证据直接证明：
...
- 因而当前最具体的未解点已经收敛成：
  ...
  - 当前真实音乐链为什么也没有命中这轮直接选中的 `sub_14206A490 / sub_14206FEF0`
  - 是因为这轮签名落到的不是当前音乐链使用的那一份实现
  - 还是因为 `sub_1426C4120` 之后的真实提交边界比目前静态命名的点更靠后
