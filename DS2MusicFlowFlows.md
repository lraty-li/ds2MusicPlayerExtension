...
- `%u.wem` 解析链上的 `sub_1426932B0` 与 `sub_142693510` 已成功挂上，并且运行时已经明确看到：
  ...
  - `sub_142693510` 与 `sub_1426C4120` 这组 resolver / low-level 结果描述链在运行时是可见的
- 但这里必须明确修正一条旧结论：
  ...
  - 当前最多只能说：游戏底层存在这样一条共享 resolver / low-level 文件定位链，而它与播放器操作在运行时存在相关现象
...
- 因此，若下一轮继续朝“额外歌曲接入”推进，当前更稳妥的表述应是：
  - 把 `sub_1426932B0 / sub_142693510` 仅保留为共享 resolver 观察点
  - 继续在 IDA 里确认 `sub_140AC5210` 下游谁才是真正进入文件定位或媒体读取的层
  - 把 `sub_1426C4120` 及其后续读设备链保留为静态候选，但不再提前写成已证实的播放器主重定向边界
