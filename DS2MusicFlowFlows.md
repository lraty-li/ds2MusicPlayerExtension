...
- `%u.wem` 解析链上的 `sub_1426932B0` 与 `sub_142693510` 已成功挂上，并且运行时已经明确看到：
  - `sub_1426932B0` 会在标题界面与真实音乐相关场景里都出现，因此它更像共享的底层包内解析尝试层，而不是播放器专用入口
  - 当前可见样本里，`sub_1426932B0` 主要走 `branch=fileId.table`，偶尔可见 `branch=name.hash`
  - 当前可见样本里，`sub_1426932B0` 的结果稳定为 `66`
  - 与之配对的 `sub_142693510` 在当前可见样本里稳定返回 `1`
  - `sub_142693510` 的 `resultKind` 当前可见样本里稳定为 `global.fileId`
  - `sub_142693510` 产出的 `outObject` 已在运行时直接看到会被紧随其后的 `sub_1426C4120` 作为 `desc0 / desc1` 消费；这说明它不是仅供观察的中间对象，而是真正进入低层读提交的结果描述
  - `sub_1426C4120` 当前可见样本里统一表现为 `source=global.fileId`，并携带 `entry / offset / size / buffer` 一类低层批量读提交信息
- 这说明当前已能用运行时证据支撑：游戏底层确实存在一条 `fileId / name -> "%u.wem" -> 包内尝试 -> 全局 fileId 后备解析` 的真实链路，而不是一开始就直接按外部完整路径打开文件。
...
- 这支持一个当前最有价值的运行时判断：同一首歌的“试听”和“正式播放”很可能对应不同的 Wwise `fileId`；它与上层 `trialSoundResource / soundResource` 分离的结构是一致的。
- `sub_1426C3B10` 当前仍没有拿到足够的运行时样本；结合静态下钻，它更像辅助命名/路径构造层，而不是当前最优先的重定向边界。
- 因此，若下一轮继续朝“额外歌曲接入”推进，主优先级应收束到：
  - 以 `sub_142693510` 作为当前主观察/主重定向候选边界
  - 把 `sub_1426932B0` 视作它前面的共享包内解析尝试层
  - 把 `sub_1426C4120` 降为“验证其下游读提交确实发生”的证据点，而不是主重定向边界
  - 继续在 IDA 里确认 `sub_1426C4120` 之后谁才是真正的低层打开点
