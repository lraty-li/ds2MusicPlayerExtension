...
- 其中关于 `"%u.wem"` / `sub_1426932B0 / sub_142693510` 的一批旧判断，后来已确认不能直接写成“播放器实际播放主链”；
  这些内容保留仅作当时的静态探索轨迹，阅读时必须以主知识页的修正结论为准。
...
### 本轮新增：`CAkFilePackageLowLevelIO` 双视图虚表的静态确认

- `sub_142691640` 已静态确认把同一个
  `CAkFilePackageLowLevelIO<CAkDefaultIOHookDeferred, CAkDiskPackage>`
  实例同时注册给 Wwise：
  - `AK::StreamMgr::SetFileLocationResolver(off_145FB6C00)`
  - `AK::StreamMgr::CreateDevice(..., &off_145FB6C08, ...)`
- 当前已对上的 resolver 视图槽位包括：
  - `+0x18 -> sub_142693510`
  - `+0x38 -> sub_1426932B0`
- 当前已对上的 low-level I/O 视图槽位包括：
  - `+0x08 -> sub_142692DE0`
  - `+0x10 -> sub_142692E90`
  - `+0x28 -> sub_142692C90`
  - `+0x30 -> sub_1426C4120`
  ...
  - `+0x48 -> sub_142692F60`
- 这进一步支持当前分层：
  - `sub_142693510 / sub_1426932B0` 是 resolver 层
  - `sub_142692C90 / sub_1426C4120 / sub_142692DE0` 是 low-level I/O 层
  - 不能再把两层混成同一个“直接打开外部文件”的函数

### 本轮新增：`qword_14619D918` 背后对象类型的静态确认

- `qword_14619D918` 现已静态确认来自：
  - `sub_140119CF0 -> sub_14206ED10`
- `sub_14206ED10` 构造出的对象类型是：
  - `NXStorageReadDevice`
- `sub_1420721B0` 则会把同一个 `qword_14619D918` 嵌入到：
  - `DecompressingReadDevice`
- 因而，当前音乐链里出现过的“委托给 `qword_14619D918` 收尾/后备”的那层，
  已不再是完全未知的共享后端，而是 `NXStorageReadDevice` 这一具体读设备。

### 本轮新增：对“存在外部路径读能力”与“音乐链已可外读”的静态拆分

- 沿 `NXStorageReadDevice` 继续静态下钻后，当前已能明确看到：
  - `sub_1427FE080(path)` 会对传入宽字符串路径执行：
    - `GetFullPathNameW`
    - 小写规范化
    - `CreateFileW`
    - `GetFileSizeEx`
  - 其下游 `sub_1427FC8D0 / sub_1427FE940 / sub_1427FFC20` 还会继续出现：
    - `SetFilePointer`
    - `ReadFile`
    - `CloseHandle`
- 这组静态证据只足以证明：
  - 游戏二进制内部确实存在“按完整外部路径打开并读取文件”的底层设备能力
- 但它仍不足以证明：
  - 当前音乐播放链 `sub_142693510(resultKind=global.fileId) -> sub_1426C4120`
    已经能够直接把任意用户目录路径交给这条设备能力
- 目前静态上已经确认的音乐主链仍然只收束到：
  - `fileId`
  - `%u.wem`
  - package/global.fileId 风格结果描述
- 因而，到本轮为止，仍然**不能**把“音乐已经能直接读取外部文件”
  作为主知识库结论写入；当前只能写成：
  - 已确认存在外部路径读设备能力
  - 但尚未确认音乐链本身能否构造并命中该能力

### 本轮新增：`sub_1426C4120` 已静态接到 `NXStorageReadDevice(+32)`

- `sub_1426C4120` 现在已经通过反汇编与对象表静态对上：
  - 指令形态是：
    - `mov rcx, cs:off_14407FB20`
    - `mov rax, [rcx]`
    - `call qword ptr [rax+20h]`
- `off_14407FB20` 指向的对象，其 `+0x20` 槽位已静态确认是：
  - `sub_14206A490`
- `sub_14206A490` 的内部行为已静态确认：
  - 先调用 `sub_14206A1B0`
  - `sub_14206A1B0` 会构造固定格式字符串：
    - `cache:streams/%x/%x/%x/%x/%s.%02x.stream`
  - 然后调用：
    - `(*qword_14619D918_vtbl + 0x20)(qword_14619D918, ...)`
- 这意味着当前音乐 low-level 提交链：
  - `sub_1426C4120`
  - 并不是停在一个未知黑盒
  - 而是已经**静态接到** `NXStorageReadDevice` 的“打开/排队”入口

### 本轮新增：音乐链已接入路径型读设备，但当前喂入的是固定 `cache:streams` 键

- `sub_14206A490 -> sub_14206A1B0` 当前喂给 `NXStorageReadDevice` 的不是用户路径字符串，
  而是由 16 字节键值 + 类型字节拼出来的固定 `cache:streams/...` 名称。
- `NXStorageReadDevice` 这一侧继续静态下钻后，当前已对上的虚表项包括：
  - `+0x20 -> sub_14206FEF0`
  - `+0x30 -> sub_142070110`
  - `+0x38 -> sub_14206EA50`
- 其中 `sub_14206FEF0` 会继续从请求描述里提取字符串键
  （通过 `sub_1400CA170`），再把该键挂入设备内部请求表。
- 同一设备类的另一虚表项 `sub_14206FC10` 已静态确认会把字符串列表逐个转成宽字符，
  然后直接调用：
  - `sub_1427FE080(path)`
  - 其下游就是 `CreateFileW / GetFileSizeEx / ReadFile`
- 但到本轮为止，**仍未**拿到下面这条最后一跳的硬证据：
  - “音乐链当前送入的 `cache:streams/...` 键，必然会在后续命中
    `sub_14206FC10 -> sub_1427FE080` 这条按路径开文件分支”
- 因而本轮修正后的结论应写成：
  - 已静态确认音乐链会进入 `NXStorageReadDevice`
  - 已静态确认该设备类具备按路径打开文件能力
  - 但当前音乐链已证明可提交的仍是固定 `cache:streams/...` 键
  - 还**不能**据此直接推出“当前音乐链已经能够读取任意用户目录文件”
