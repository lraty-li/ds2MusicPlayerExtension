const CLIENT_ID_KEY = "ds2.spotify.webview2Poc.clientId";
const TOKEN_KEY = "ds2.spotify.webview2Poc.token";
const VERIFIER_KEY = "ds2.spotify.webview2Poc.pkceVerifier";
const STATE_KEY = "ds2.spotify.webview2Poc.oauthState";
...
export function saveClientId(value) {
  ...
  if (previous && previous !== clientId) localStorage.removeItem(TOKEN_KEY);
...
export async function finishAuthorizationCallback() {
  ...
  if (!verifier) throw new Error("缺少 PKCE verifier，请重新授权");
...
async function requestToken(parameters) {
  ...
  if (!response.ok || !body.access_token) {
    throw new Error(
      body.error_description || body.error || `Token 请求失败：HTTP ${response.status}`
    );
  }
  return body;
}
