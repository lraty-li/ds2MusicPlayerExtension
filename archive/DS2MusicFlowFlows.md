...
- 曾经有一组运行时样本，把一个“与 `sub_14206FEF0` 形态一致的提交节点”与 `sub_1426C4120` 关联起来：
  - 该节点返回的 `outHandle`
  - 会在紧随其后的 `sub_1426C4120 item` 里再次出现为 `streamHandle`
  - 同时该节点的 `callbackCtx` 会对上该 item 的 `segment`
- 这至少说明，`sub_1426C4120` 之后确实存在一个把句柄与 `segment` 绑定在一起的提交边界。
- 但这里也必须明确修正另一条错误前提：
...
- 新一轮围绕 `sub_1426C4120` 提交侧字段的观测又补上了一层运行时闭环：
  - 对目标曲目命中 `sub_1426C4120` 时，已经能直接记录：
    - `submitCtx`
    - `streamObject`
    - `wemResource`
    - `wemId`
    ...
    - `readBytes`
    - `dstBuffer`
  - 这些字段在试听第二首、正式播放第一首、正式播放第二首、再次正式播放第一首几组样本里都稳定出现
  - 其中 `logicalOffset` 已稳定出现 `0 / 131072` 两段推进，`readBytes` 稳定为 `131072`
- 因而当前应把 `sub_1426C4120` 的表述进一步收紧为：
  - 它不只是“上游 `wemResource / wemId / streamHandle / segment` 观测点”
  - 而是当前已经运行命中并拿到“资源级绑定 + 输出缓冲 + 写入长度 + 逻辑偏移”的提交边界
- 但同一轮也明确否掉了一条新的错误前提：
...
- 因此，若继续朝“额外歌曲接入”推进，当前更稳妥的表述应是：
  - 把 `sub_142693510 -> sub_1426C4120` 视为当前已证实的主线骨架
  - 把“与 `sub_14206FEF0` 形态一致的提交层”仅视为已观察过的候选边界，而不是已经钉死的静态函数本体
  - 后续跟踪应优先使用 `wemId / wemResource / streamHandle`，而不是只靠短时间窗
- 进一步的运行验证又补上了两条反证：
  - 即使把 submit route hook 的安装时机前移到 resolver / `sub_1426C4120` 之前，当前选中的 `sub_14206A490 / sub_14206FEF0` 仍然零命中
  - 即使直接 hook `KernelBase!ReadFile`，真实的试听、正式播放、下一首流程里也没有任何命中日志
  - 同时 `sub_1426C4120` 的目标 `segment` 记录仍然稳定成功
- 这说明当前还不能把静态命名的 `sub_14206A490 / sub_14206FEF0`，或者 `KernelBase!ReadFile`，直接当成真实音乐链已经落到的实现边界。
- 当前更合理的待确认对象，已经收束到：
  - `off_14407FB20->vftable + 0x20` 的运行时真实函数指针
  - `qword_14619D918->vftable + 0x20` 的运行时真实函数指针
- 新一轮还补上了新的运行时反证：
  - `sub_142692E90` 与 `sub_142692EE0` 的 hook 都成功安装
  - 但在真实的试听、正式播放、下一首流程里真正命中数仍为 `0`
  - 因而当前不能把它们继续当成音乐分段请求的实际完成 / 释放主路径
- 同一轮也补上了一个仍未闭环的现象：
  - 已经在 `sub_1426C4120` 命中目标提交样本
  - 但没有记录到任何 `dynamic submitter.slot4` 安装日志或 `submitter.slot4` 命中日志
  - 因而当前还不能把“运行时 submitter 槽位 `[4]` 的 detour 已成功挂上”写成既定事实
- 更进一步的 submitter 观测又把失败原因继续收窄：
  - 当前代码按 `base + 0x407FB20` 读取所谓 submitter 全局槽位时
