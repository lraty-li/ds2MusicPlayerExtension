...
任何IDA MCP 探索都交给子agent来做，避免挤满上下文， 委派模型： gpt-5.2 xhigh， 必须是 gpt-5.2， 不能是 gpt-5.4

## 子 agent 模型合规闸门
- 任何 IDA MCP 探索在创建子 agent 时，必须显式指定 `model=gpt-5.2` 与 `reasoning_effort=xhigh`
- 仅仅“参数里写了 gpt-5.2”不算满足要求；必须再核对创建结果或界面展示，确认实际子 agent 明确显示为 `gpt-5.2`
- 只要创建结果、界面展示、系统返回信息里出现 `gpt-5.4`、未显示具体模型、或存在模型被角色/平台默认值覆写的歧义，都视为本次委派失败，不得继续使用该子 agent 产出
- 若出现上述失败，必须立刻停止该子 agent，并明确向用户报告“本次委派模型不合规”，然后改用能够被明确验证为 `gpt-5.2 xhigh` 的方式重新创建
- 对于 IDA MCP 探索，不得为了使用某个 `agent_type` 而放松模型约束；只要某种创建方式存在把模型回退、覆写或展示成其他模型的风险，就禁止继续使用该方式
- 若平台当前无法提供“可被明确验证为 `gpt-5.2 xhigh`” 的子 agent，则必须暂停该轮 IDA 探索并先向用户说明，不能以其他模型代替
